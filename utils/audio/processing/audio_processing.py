...
def blend_chunks(chunk1, chunk2, overlap):
    ...
    for i in range(overlap):
        alpha = i / overlap  # Blend factor from 0 to 1
        blended_chunk[-overlap + i] = (1 - alpha) * chunk1[-overlap + i] + alpha * chunk2[i]
    return np.vstack((blended_chunk, chunk2[overlap:]))
...
def process_audio_features(audio_features, model, device, config):
    ...
    if len(np.concatenate(all_decoded_outputs, axis=0)) < num_frames:
        remaining_frames = num_frames - len(np.concatenate(all_decoded_outputs, axis=0))
